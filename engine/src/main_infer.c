...
static int parse_flags(int argc, char **argv,
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (!strcmp(arg, "--help") || !strcmp(arg, "-h")) {
      print_usage(stdout); exit(0);
    } else if (!strcmp(arg, "--prompt")) {
      if (++i >= argc) { fprintf(stderr, "error: --prompt requires a value\n"); return -1; }
      prompt = argv[i];
    } else if (!strcmp(arg, "--max-new")) {
      if (++i >= argc) { fprintf(stderr, "error: --max-new requires a value\n"); return -1; }
      long v = strtol(argv[i], NULL, 10);
      if (v < 0) { fprintf(stderr, "error: --max-new must be >= 0\n"); return -1; }
      max_new = (uint32_t)v;
    } else if (!strcmp(arg, "--threads")) {
      if (++i >= argc) { fprintf(stderr, "error: --threads requires a value\n"); return -1; }
      long v = strtol(argv[i], NULL, 10);
      if (v < 0) { fprintf(stderr, "error: --threads must be >= 0\n"); return -1; }
      p->threads = (uint32_t)v;
    } else if (!strcmp(arg, "--precision")) {
      if (++i >= argc) { fprintf(stderr, "error: --precision requires a value\n"); return -1; }
      const char *val = argv[i];
      if (strcmp(val,"fp32") && strcmp(val,"bf16") && strcmp(val,"fp16")) {
        fprintf(stderr, "error: --precision must be fp32|bf16|fp16\n"); return -1;
      }
      p->precision = val;
    } else if (!strcmp(arg, "--affinity")) {
      if (++i >= argc) { fprintf(stderr, "error: --affinity requires a value\n"); return -1; }
      const char *val = argv[i];
      if (strcmp(val,"auto") && strcmp(val,"compact") && strcmp(val,"scatter")) {
        fprintf(stderr, "error: --affinity must be auto|compact|scatter\n"); return -1;
      }
      p->affinity = val;
    } else if (!strcmp(arg, "--grainsize")) {
      if (++i >= argc) { fprintf(stderr, "error: --grainsize requires a value\n"); return -1; }
      long v = strtol(argv[i], NULL, 10);
      if (v < 0) { fprintf(stderr, "error: --grainsize must be >= 0\n"); return -1; }
      p->grainsize = (uint32_t)v;
    } else if (!strcmp(arg, "--numa")) {
      if (++i >= argc) { fprintf(stderr, "error: --numa requires a value\n"); return -1; }
      p->numa_mode = argv[i];
    } else if (!strcmp(arg, "--pretranspose")) {
      if (++i >= argc) { fprintf(stderr, "error: --pretranspose requires a value\n"); return -1; }
      const char *val = argv[i];
      if (strcmp(val,"none") && strcmp(val,"woh") && strcmp(val,"wxh") && strcmp(val,"all")) {
        fprintf(stderr, "error: --pretranspose must be none|woh|wxh|all\n"); return -1;
      }
      p->pretranspose = val;
    } else if (arg[0] == '-') {
...
int main(int argc, char **argv) {
  ...
  if (parse_flags(argc, argv, &params, &prompt, &max_new) != 0) return 2;

  ie_engine_t *eng = NULL;
  if (ie_engine_create(&params, &eng) != IE_OK) return 3;

  uint32_t *tokens = NULL;
  if (max_new > 0) {
    tokens = (uint32_t*)malloc(sizeof(uint32_t) * max_new);
    if (!tokens) { ie_engine_destroy(eng); return 4; }
  }

  const double t0 = now_s();
  uint32_t out_count = 0;
  if (ie_engine_generate(eng, prompt, max_new, tokens, &out_count) != IE_OK) {
    free(tokens); ie_engine_destroy(eng); return 5;
  }
  const double t1 = now_s();

  ie_metrics_t m;
  if (ie_engine_metrics(eng, &m) != IE_OK) { free(tokens); ie_engine_destroy(eng); return 6; }

  fputs("{", stdout);
  fprintf(stdout, "\"tokens_generated\": %u", (unsigned)out_count);
  fputs(",\"tokens\": [", stdout);
  for (uint32_t i = 0; i < out_count; ++i) fprintf(stdout, "%s%u", (i ? "," : ""), tokens[i]);
  fputs("]", stdout);
