/**
 * @file numa_policy.c
 * @brief NUMA-aware policy initialization and helpers.
 *
 * This module:
 *  - detects sockets via ie_topology (if available),
 *  - detects NUMA nodes via sysfs (ie_numa_detect_nodes),
 *  - reads env knobs,
 *  - provides decisions used by the loader and hot replication code.
 */
 ...
 * @return 1 if equal (case-insensitive), else 0.
 */
...
/**
 * @brief Read a boolean-like env var.
 *
 * Accepted false values (case-insensitive): "0", "false", "no", "off".
 * Anything else that is set is treated as true.
 *
 * @param name Environment variable name.
 * @param default_value Value used when not set.
 * @return 0 or 1.
 */
...
/**
 * @brief Read an integer env var with clamping.
 *
...
/**
 * @brief Read a size_t env var with clamping.
 *
 * @param name Env var name.
 * @param def Default if unset or unparsable.
 * @param min_v Minimum allowed value.
