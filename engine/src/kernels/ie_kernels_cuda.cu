...
/* Stream type mapping (matches the typedef in the public header). */
...
#error "This file must be compiled with NVCC as a CUDA translation unit."
#endif
...
static inline void ie_cuda_set_err(const char *msg) {
  ...
  size_t i = 0;
  for (; i + 1 < sizeof(g_ie_cuda_err) && msg[i]; ++i) {
    g_ie_cuda_err[i] = msg[i];
  }
  g_ie_cuda_err[i] = '\0';
}
 ...
 * and reduce via shared memory. Assumes `ldW >= cols`.
 */
...
/**
 * @brief Row-wise GEMV with FP8 activations (E4M3/E5M2), fused decode.
 */
...
/**
 * @brief Pack row-major W into Blocked-K layout on device.
 *
 * Layout:
 *   kb = k / block_k;  // tile index along K
 *   ko = k % block_k;  // in-tile offset
...
int ie_cuda_launch_gemv_bias_act_f32(const float *W,
  ...
  dim3 block(256, 1, 1);
  dim3 grid(rows, 1, 1);
  k_gemv_bias_act_f32<<<grid, block, 0, stream>>>(W, x, bias, y,
                                                  rows, cols, ldW,
                                                  alpha, beta, act);
  CUDA_GUARD(cudaGetLastError());
  return IE_CUDA_OK;
...
int ie_cuda_launch_gemv_rowwise_qi8_f32(const float   *W,
  ...
  dim3 block(256, 1, 1);
  dim3 grid(rows, 1, 1);
  k_gemv_rowwise_qi8_f32<<<grid, block, 0, stream>>>(W, xq, y,
                                                     rows, cols, ldW,
                                                     scale, zp,
                                                     alpha, beta);
  CUDA_GUARD(cudaGetLastError());
...
int ie_cuda_launch_gemv_rowwise_qfp8_f32(const float   *W,
  ...
  dim3 block(256, 1, 1);
  dim3 grid(rows, 1, 1);
  k_gemv_rowwise_qfp8_f32<<<grid, block, 0, stream>>>(W, x8, y,
                                                      rows, cols, ldW,
                                                      fmt, alpha, beta);
  CUDA_GUARD(cudaGetLastError());
  return IE_CUDA_OK;
...
int ie_cuda_launch_pack_w_blockedk_f32(float       *Wp,
  ...
  k_pack_w_blockedk_f32<<<grid, block, 0, stream>>>(Wp, W,
                                                    rows, cols,
                                                    ldW, block_k);
  CUDA_GUARD(cudaGetLastError());
  return IE_CUDA_OK;
