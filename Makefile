...
# Use '>' as recipe prefix (avoids hard tab requirement).
.RECIPEPREFIX := >
...
# CPU-only CUDA stubs (avoid pulling CUDA runtime into the CPU binary)
SRC_CPU_STUB_C := \
...
.PHONY: build build-release build-cuda build-all cuda \
	test bench bench-direct bench-cuda cuda-bench \	profile perf-report baseline-report \	fmt lint docs docs-doxygen clean microbench microbench-stream microbench-block-sparse \	monitoring-up monitoring-down metrics-exporter \	ptq-calibrate ptq-from-hf ptq-from-torch ptq-from-bin \	perf_cpu_fp32 perf_cpu_int8 perf_cpu_bf16 perf_gpu \	perf_cpu_act_int8 perf_gpu_act_fp8_e4m3 perf_gpu_act_fp8_e5m2 \	show-tools iebin model-pack pack-hf refresh-model repack-hf \	convert-to-block-sparse
cuda: build-cuda
...
> @set -e; set -o pipefail; \
        ...
        RUNS_VAL="$${RUNS:-3}"; \
        ROUNDS_VAL="$${ROUNDS:-1}"; \
        echo "[bench] strict run (true TPS)… (RUNS=$$RUNS_VAL, ROUNDS=$$ROUNDS_VAL)"; \
...
> @set -e; set -o pipefail; \
        ...
        RUNS_VAL="$${RUNS:-3}"; \
        ROUNDS_VAL="$${ROUNDS:-1}"; \
        echo "[bench-cuda] strict run (true TPS)… (RUNS=$$RUNS_VAL, ROUNDS=$$ROUNDS_VAL)"; \
        env -i PATH="$$PATH" SHELL="$$SHELL" HOME="$$HOME" \
